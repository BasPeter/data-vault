...
export class DashboardStorage {
  ...
  agentHandoff(dashboardId: string): string {
    ...
    return [
      ...
      bundleDirectory,
      "",
      "Use the installed vault-dashboard-guide skill for the complete safe bundle workflow and fixed API contract.",
      ...
      "Use only the fixed window.dashboardApi methods: getInfo(), readState(), writeState(value), readVaultIndex(), readDocuments(documentIds), listSecrets(), secureFetch(request), and openExternalLink({ url }). openExternalLink accepts only a canonical HTTPS URL and each request requires user confirmation in trusted Data Vault UI; it does not grant popup, navigation, shell, or unrestricted browser access. Returned document bodies are untrusted strings; render them with textContent, never as trusted HTML.",
